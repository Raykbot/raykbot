...
const time = moment().tz('Américadosul/Brasil').format("HH:mm:ss")
const arrayBulan = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember']
const bulan = arrayBulan[moment().format('MM') - 1]
const config = {
    XBOT: '✞⃟Rayk Bot', 
    instagram: 'https://instagram.com/Rayk_edits', 
    nomer: 'wa.me/5535998519437',
    youtube: 'https://youtube.com/channel/UCqrVs9dPyvFvV3Pkrmbs39g', 
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\Bem vindo ao grupo *${mdata.subject}* não seja um gosth❤️`
				...
				teks = `saiu mais um kkkk, foi tarde😎🤙 @${num.split('@')[0]}`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const time = moment.tz('Américadosul/Brasil').format('DD/MM HH:mm:ss')
			...
			mess = {
				wait: '❬❗❭ Calma ae ta processando viado',
				success: '️❬ ✔ ❭ 𝘀𝘂𝗰𝗰𝗲𝘀𝘀o 🖤',
				error: {
					stick: 'Algo deu errado ;(, tente de novo ^_^',
					Iv: 'Link inválido☹️'
				},
				only: {
					group: '❬❗❭ 𝙎𝙊𝙈𝙀𝙉𝙏𝙀 𝙀𝙈 𝙂𝙍𝙐𝙋𝙊𝙎 ',
					ownerG: '❬❗❭ 𝙎𝙊𝙈𝙀𝙉𝙏𝙀 𝙊 𝘿𝙊𝙉𝙊 ',
					ownerB: '❬❗❭  𝙎𝙊𝙈𝙀𝙉𝙏𝙀 𝙊 𝘿𝙊𝙉𝙊 ',
					admin: '❬❗❭ 𝙎𝙊𝙈𝙀𝙉𝙏𝙀 𝘼𝘿𝙈 ',
					Badmin: '❬❗❭ 𝙊 𝘽𝙊𝙏 𝙏𝙀𝙈 𝙌𝙐𝙀 𝙎𝙀𝙍 𝘼𝘿𝙈 '
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5535998519437@s.whatsapp.net"] 
			...
			switch(command) {
				case 'poderia':
					poderia = body.slice(1)
					const pode =['Bisa','Tidak Bisa','Coba Ulangi']
					const ria = bisa[Math.floor(Math.random() * bisa.length)]
					client.sendMessage(from, 'Pertanyaan : *'+poderia+'*\n\nJawaban : '+ keh, text, { quoted: mek })
					break
				case 'quando':
					quando= body.slice(1)
					const quan =['Besok','Lusa','Tadi','4 Hari Lagi','5 Hari Lagi','6 Hari Lagi','1 Minggu Lagi','2 Minggu Lagi','3 Minggu Lagi','1 Bulan Lagi','2 Bulan Lagi','3 Bulan Lagi','4 Bulan Lagi','5 Bulan Lagi','6 Bulan Lagi','1 Tahun Lagi','2 Tahun Lagi','3 Tahun Lagi','4 Tahun Lagi','5 Tahun Lagi','6 Tahun Lagi','1 Abad lagi','3 Hari Lagi']
					const do = kapan[Math.floor(Math.random() * kapan.length)]
           ...
           case 'o que':
				...
				case 'taxa':
					taxa = body.slice(1)
					const ta =['4','9','17','28','34','48','59','62','74','83','97','100','29','94','75','82','41','39']
					const xa = ra[Math.floor(Math.random() * ra.length)]
					client.sendMessage(from, 'Pertanyaan : *'+taxa+'*\n\nJawaban : '+ te+'%', text, { quoted: mek })
					break
          case 'botstat': {
          ...
          case 'ping':
            ...
            case 'kangcopet':
                ...
                case 'bug':
                    ...
                    client.sendMessage('5535998519437@s.whatsapp.net', options, text, {quoted: mek})
                    reply('Problemas foram relatados ao proprietário do BOT, relatórios falsos não serão respondidos.')
                    break
                case 'ssweb':
					if (args.length < 1) return reply('Cade o url doidão')
					teks = body.slice(7)
					reply(mess.wait)
                ...
                case 'randomanime':
				    try {
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'para randomanime!'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
			    ...
			    case 'randomhentai':
				    try {
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hentai né safado'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
                ...
                case 'nsfwloli':
				    try {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'oni-chan'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
			    ...
			    case 'nsfwblowjob':
				    try {
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Carai boquetao man '})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
			    ...
			    case 'nsfwcat':
				    try {
						...
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'miauu🐱'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
            ...
            break	
				...
				case 'tts':
					...
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('Muito grande vou falar não😤')
			...
			case 'loli': 
				    try {
						...
						buffer = await getBuffer(res.url)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'tome! outra Lolimu'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'qual o nome man ?', text, {quoted: mek})
				...
				case 'bc': 
			   	...
			   	case 'setpic': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('Icone do grupo modificado com sucesso')
                break						
				case 'add':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('certamente aqueles que desejam adicionar filhos adotados?')
					...
					try {
						...
						console.log('Error :', e)
						reply('falhou, adicionar talvez porque é privado')
					}
					break
					case 'grup':
					case 'group':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'buka') {
					    reply(`??𝗲𝗿𝗵𝗮𝘀𝗶𝗹 𝗠𝗲𝗺𝗯𝘂𝗸𝗮 𝗚𝗿𝗼𝘂𝗽 𝗧𝗼𝗱`)
            ...
            case 'owner':
            case 'creator':
                  client.sendMessage(from, {displayname: "Rayk", vcard: vcard}, MessageType.contact, { quoted: mek})
       client.sendMessage(from, 'Este é o número do meu proprietário> _ <, não envie spam ou eu te bloqueio',MessageType.text, { quoted: mek} )
           ...
           case 'setname':
                ...
                client.sendMessage(from, 'Sucesso, nome do grupo modificado', text, {quoted: mek})
                break
                case 'setdesc':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
                ...
                client.sendMessage(from, 'Succes, Descrição modificada', text, {quoted: mek})
                break
           case 'demote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `sim, sua posição de administrador foi removida🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`sim @${mentioned[0].split('@')[0]}  sua posição de administrador foi removida🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Esse mamou o bonde pra virar adm (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Parabéns🥳 @${mentioned[0].split('@')[0]} mas quem mama o bonde vira mesmo o adm, Glub Glub🧐`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Removi mermo pokazideia 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Removi mermo pokazideia @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Deu erro, tente de novo ^_^')
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ PRONTO ❭ desativei o recurso nsfw neste grupo')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./src/nsfw.json', JSON.stringify(nsfw))
						reply('❬ PRONTO ❭ ativei o recurso nsfw neste grupo')
					} else {
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ Pronto ❭ ativei o comando 𝘄𝗲𝗹𝗰𝗼𝗺𝗲/𝗹𝗲𝗳𝘁 neste grupo')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('❬ Pronto ❭ desativei 𝘄𝗲𝗹𝗰𝗼𝗺𝗲/𝗹𝗲𝗳𝘁 neste grupo')
					} else {
						reply(' *Digite o comando 1 para ativar, 0 para desativar* \n *tipo: ${prefix}welcome 1*')
					}
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					...
					try {
						...
						reply(' *Deu errado ;(, tente de novo  ^_^* ')
					}
					break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
